...
HttpClient.prototype.isExpired = function(authToken) {
    ...
    var refreshLimit = new Date(new Date(expiration.getFullYear(), expiration.getMonth(), expiration.getDate(), expiration.getHours(), expiration.getMinutes() - 10)).getTime();

    return (now >= refreshLimit);
};

HttpClient.prototype.get = function(resourceUrl) {
    var options = {
        url: resourceUrl,
        method: 'GET',
        headers: {}
...
HttpClient.prototype.put = function(resourceUrl, data) {
    var options = {
        url: resourceUrl,
        method: 'PUT',
        headers: {},
...
function AuthTokenError(message) {
    this.name = 'AuthTokenError',
        this.message = message || 'An error occurred retreiving the authentication token';
}
