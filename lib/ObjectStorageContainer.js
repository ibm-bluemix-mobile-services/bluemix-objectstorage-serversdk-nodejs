 ...
 *  @param {} data. The content of the object to store
 *  @return promise
 */
ObjectStorageContainer.prototype.createObject = function(objectName, data) {
    var deferred = Q.defer();
    var self = this;
    var resourceUrl = this.baseResourceUrl + '/' + objectName;

    this.client.put(resourceUrl, data).then(function(res) {
        var object = new ObjectStorageObject(objectName, self);
        deferred.resolve(object);
    }).catch(function(err) {
        deferred.reject(err);
...
ObjectStorageContainer.prototype.deleteObject = function(objectName) {
    ...
    this.client.delete(resourceUrl).then(function(response) {
        deferred.resolve();
    }).catch(function(err) {
        deferred.reject(err);
    });
